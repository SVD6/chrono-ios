...
class StopwatchManager: ObservableObject {
    ...
    func start() {
        ...
        self.stopwatch = Timer.scheduledTimer(withTimeInterval: 0.01, repeats: true) { _ in
            self.totalTime += 0.01
    ...
    @objc fileprivate func willEnterForegroundNotification() {
        print("Foreground")
        self.background_forground_timelaps = Date().timeIntervalSince(self.backgroundTime ?? Date())
        self.backgroundTime = nil
        self.totalTime += background_forground_timelaps ?? 0
        self.currentLap += background_forground_timelaps ?? 0
    }
